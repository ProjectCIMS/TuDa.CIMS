...
public partial class ShoppingCartSubmitPopup
{
    ...
    [Parameter] public List<WorkingGroup> WorkingGroups { get; set; }
    ...
    private static string GetPriceText(AssetItem assetItem) =>
        assetItem switch
        {
            Substance substance => $"{substance.Price}€/" + substance.PriceUnit switch
            {
                PriceUnits.PerKilo => "kg",
                PriceUnits.PerLiter => "l",
                PriceUnits.PerPiece => "Stück",
                _ => $"{substance.Price}€ {substance.PriceUnit}"
            },
            _ => $"{assetItem.Price}€/Stück"
        };
}
