...
public interface IAssetItemApi
{
    ...
    public async Task<ErrorOr<IEnumerable<AssetItem>>> GetAllAsync(
        AssetItemFilterDto? filter = null
    )
    {
        return await GetAllAsyncInternal(filter).ToErrorOrAsync();
    }
    ...
    protected Task<IApiResponse<IEnumerable<AssetItem>>> GetAllAsyncInternal(
        [Query] AssetItemFilterDto? filter
    );
    ...
    public async Task<ErrorOr<Created>> CreateAsync(CreateAssetItemDto createModel) =>
        await CreateAsyncInternal(JsonSerializer.Serialize(createModel)).ToErrorOrAsync<Created>();
    ...
    protected Task<IApiResponse> CreateAsyncInternal([Body] string createModel);
