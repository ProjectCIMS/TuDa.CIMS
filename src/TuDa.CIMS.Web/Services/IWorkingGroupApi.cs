...
public interface IWorkingGroupApi
{
    ...
    /// Uses <see cref="JsonSerializer"/> to ensure proper serialization of polymorphic types.
    /// </remarks>
    public async Task<ErrorOr<Updated>> AddStudentsAsync(
        Guid id,
        UpdateWorkingGroupDto updateWorkingGroupDto
    ) =>
        await UpdateAsyncInternal(id, JsonSerializer.Serialize(updateWorkingGroupDto))
    ...
    [Patch("/{id}")]
    [Headers("Content-Type: application/json; charset=utf-8")]
    protected Task<IApiResponse> AddStudentsAsyncInternal(Guid id, [Body] string updateWorkingGroupDto);
    ...
    /// Uses <see cref="JsonSerializer"/> to ensure proper serialization of polymorphic types.
    /// </remarks>
    public async Task<ErrorOr<Updated>> DeleteStudentsAsync(
        Guid id,
        UpdateWorkingGroupDto updateWorkingGroupDto
    ) =>
        await DeleteStudentsAsyncInternal(id, JsonSerializer.Serialize(updateWorkingGroupDto))
            .ToErrorOrAsync<Updated>();
    ...
    [Patch("/{id}")]
    [Headers("Content-Type: application/json; charset=utf-8")]
    protected Task<IApiResponse> DeleteStudentsAsyncInternal(Guid id, [Body] string updateWorkingGroupDto);
}
